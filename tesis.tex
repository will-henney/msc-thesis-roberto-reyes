...
En este gas ideal vamos a tomar la velocidad del sonido por su definición a una entropía constante
\[c_s = \sqrt{\Big(\frac{\partial P}{\partial\rho}\Big)_S} = \sqrt{\frac{k_B T}{\bar{m}}}\] de esta manera vemos que la velocidad del sonido en el gas ionizado solo depende de su temperatura. En este tipo de gases la velocidad del sonido es típicamente del orden de \SI{e6}{cm.s^{-1}}
...
\[\dot{n}_{rec}=n_e n_i \alpha_\mathrm{B}\] donde $\alpha_\mathrm{B}$ es el coeficiente de recombinación para el caso B. Este coeficiente de recombinación para el caso B toma en cuenta las recombinaciones a todos los niveles, excepto al nivel fundamental, esto ya que el fotón liberado en esta recombinación al nivel fundamental puede volver a ionizar algún otro átomo.
...
En estas estimaciones de las diferentes fuerzas solo haremos aproximaciones y para el caso de los valores de los glóbulos usaremos solo los más típicos, la justificación de usar estos valores típicos, la media de los valores, es que para los mejores ajustes tenemos valores similares en general.

En este modelo consideramos que la principal fuerza que se ejerce es por la diferencia de presiones en el gas ionizado. Por lo que la aceleración provocada por esta fuerza esta dada como \[a = \frac{\partial P}{\partial m/V h_0}\] donde $P$ es la presión, $m$ la masa, $V$ el volumen, por lo que $m/V$ es la densidad y $h_0$ un ancho efectivo definido como \[n_0h_0=\int n(r) dr\] y este lo calculamos de la siguiente manera. Usando las ecuaciones de la sección \ref{Estructura} tenemos que
\[\frac{\rho}{\rho_0}=e^{\frac{1-M^2}{2}}\] y \[\frac{r}{r_0}=M^{-1/2}e^{\frac{M^2-1}{4}}\]
por lo que si derivamos esta segunda ecuación con respecto al número de Mach tenemos que
\[\frac{dr}{dM}=r_0\Big( e^{\frac{M^2-1}{4}}\Big(-\frac{M^{-3/2}}{2}+\frac{M}{2 M^{1/2}}\Big)\Big)=\frac{r_0}{2}e^{\frac{M^2-1}{4}}\Big(M^{\frac{1}{2}}-M^{-\frac{3}{2}}\Big)\]
por lo que ahora podemos calcular $h_0$ como
\[h_0=\int_0^\infty \Big(\frac{n(r)}{n_0}\Big)^2 dr=\int_1^\infty \frac{r_0}{2} (exp(1-M^2))^{1-\frac{1}{4}}\Big(M^{\frac{1}{2}}-M^{-\frac{3}{2}}\Big) dM\]
el cual si integramos de manera numérica nos da un resultado $\sim 0.12r_0$. Entonces tenemos que la aceleración debido al gradiente de presión es \[a=\frac{\partial P}{\partial\rho \;h_0}=\frac{c_s^2}{h_0}=\frac{c_s^2}{0.12 r_0}\] la cual a un radio $r_0$ típico, $\sim\SI{0.13}{\arcsecond}$ tenemos una aceleración de $\SI{4.41e-5}{cm/s^{2}}$
