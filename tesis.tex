...
\begin{equation}
    ...
    \qquad
    x_\mathrm{H}^1=x+\epsilon_\mathrm{H}^i
\end{equation}
...
Si las distribuciones son simétricas, esperamos que $\mu \approx 0$ para $N$ grande, y verificamos esto para nuestras muestras.
Entonces, suponiendo que no haya correlación entre los errores $\epsilon_\mathrm{J}^i$ y $\epsilon_\mathrm{H}^i$, tenemos que
\begin{equation}
...
Si además suponemos que $\sigma_\mathrm{J} = \sigma_\mathrm{H} \equiv   \sigma$, tenemos que $\mathrm{Var}(x_\mathrm{J}^i-x_\mathrm{H}^i)=2\sigma^2\Rightarrow\sigma=\sqrt{\mathrm{Var}(x_\mathrm{J}^i-x_\mathrm{H}^i)}/\sqrt{2}$, la cual usamos 
como el estimado del incertidumbre en estas mediciones en la sección~\ref{sec:error-shell}.
...
\begin{equation}
    x^2+y^2+z^2=r^2,
\end{equation}
podemos usar las variables $X=x /r$, $Y=y/r$ y $Z=z/r$ por simplicidad y así trabajar con la esfera unitaria. En este análisis vamos a considerar que los vectores de posición y velocidad son paralelos, por lo que podemos usar las variables $X$, $Y$ y $Z$ tanto para la posición como para la velocidad. 
