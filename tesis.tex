...
Dadas las observaciones hemos estimado algunos parámetros como sus distintos radios y propiedades, sobre todo de la cáscara chocada, pero en nuestro modelos incluye mucho acerca de la parte neutra y la cáscara chocada. 

Ya hemos hecho una medición de los distintos radios, así que ahora vamos a tratar de comparar la  presión de la parte interna y de la cáscara.
...
\subsection{Medición de errores en las observaciones}

Como el radio de la parte neutra $r_0$ y el ancho de la cáscara $\mathrm{H}_s$ se midieron usando la sigma de las gaussianas ajustadas a los perfiles, tuvieron que ser corregidos por lo que primero lo relacionamos con la anchura a altura media y después se corrigió tomando en cuenta la anchura del telescopio.

Para el caso de la parte interna tenemos una medición para el radio en la parte neutra con un combinación de filtros del JWST y una aproximación el HST, ya que en este último vemos solo el gas ionizado. Aunque no hay una buena correlación entre estas dos mediciones, su valor promedio y desviación estándar se parecen mucho, por lo que vamos a considerar un radio constante que será igual para todos los glóbulos el cuál será el promedio de los valores típicos de estas dos  mediciones con un error de $\pm\SI{3}{\arcsecond}$.

Para el caso del ancho y radio de la cáscara vamos a considerar un error relativo para cada glóbulos usando las mediciones de ambos telescopios. Los errores estarán dados de la siguiente manera 
\[\epsilon_{r_s} = \frac{\frac{\sigma\Big(\mathrm{r}_{s,HST}-\mathrm{r}_{s,JWST}\Big)}{\sqrt{2}}}{\bar{r}_{s,HST}}\]
y \[\epsilon_{H_s} = \frac{\frac{\sigma\Big(\mathrm{H}_{s,HST}-\mathrm{H}_{s,JWST}\Big)}{\sqrt{2}}}{\bar{H}_{s,HST}}\]
donde $\mathrm{r_{s,JWST}}$ son las mediciones de la cáscara con el JWST, $\mathrm{r_{s,HST}}$ son las mediciones de la cáscara con el HST y $\bar{r}_{s,HST}$ el valor promedio de las mediciones del radio de la cáscara medido con el HST.

Para los errores relativos en los brillos tanto de la parte interna como de la cáscara se tomaron como
\[\epsilon_{r,B}= \frac{\sqrt{\sigma\Big((y-f(x))^2,w_1*w_2\Big)}}{B}\] 
donde $y-f(x)$ son los valores observados menos el ajuste, $w_1$ es el peso considerado en el modelo (Sección \ref{Sec : Ajuste de modelo}) y $w_2$ es la gaussiana ajustada a la parte correspondiente, es decir, para la parte interna $w_2$ es la primer gaussianan ajustada, la cual tiene su pico en \SI{0}{\arcsecond} (Sección \ref{Sec : Ajuste de modelo}) y para la cáscara $w_2$ es la segunda gaussiana ajustada. Finalmente $B$ es el pico del brillo a la parte correspondiente.
